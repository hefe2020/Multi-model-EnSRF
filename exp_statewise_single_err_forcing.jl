...
local_matrix, model_step_ml_batch, ensemble_cov, diagonal_Q_basis, build_A_for_Q_basis, 
forcing_vector, l96_rhs_forcing, forecast_lorenz_one_step, 
choose_obs_indices, build_H_from_obs_idx, estimate_diag_q_rank_deficient, 
estimate_lambda_from_error_buffers, compute_ABC_lambda, 
statewise_error_stats, statewise_lambda, compute_statewise_lambda, 
smooth_statewise_lambda, hybrid_mean_statewise
...
include("plotting.jl")
import .plotting: statewise_single_forcing
...
function run_statewise_hybrid_single_setup()
    N = 40
    dt = 0.05
    timesteps = 1000
    obs_steps = 5
    ...
    # state-wise lambda controls
    λ_min_samples = 5
    λ_window = 20
    λ_smooth_time = 0.8
    λ_smooth_radius = 1
    λ_init_state = fill(0.5, N)
    ...
    F_wrong_vec = forcing_vector(7.2, 6.5, 6.6, 7.9, N)
    ...
    λ_state_current = copy(λ_init_state)
    ...
    for k in 1:timesteps
        ...
        if k in obs_idx_all
            ...
            # rolling buffer for state-wise λ estimaton
            push!(err_lor_buf, e_lor)
            push!(err_ml_buf,  e_ml)

            if length(err_lor_buf) > λ_window
                popfirst!(err_lor_buf)
                popfirst!(err_ml_buf)
            end

            # estimate state-wise λ
            if length(err_lor_buf) >= λ_min_samples
                E_lor_buf = hcat(err_lor_buf...)
                E_ml_buf  = hcat(err_ml_buf...)

                λ_state_raw, A_state, B_state, C_state = compute_statewise_lambda(E_lor_buf, E_ml_buf)
                λ_state_raw = smooth_statewise_lambda(λ_state_raw; radius=λ_smooth_radius)

                λ_state_current = λ_smooth_time .* λ_state_current .+ (1.0 - λ_smooth_time) .* λ_state_raw
            ...
            # state-wise hybrid forecast
            xbar_hyb_f = hybrid_mean_statewise(xbar_lor_f, xbar_ml_f, λ_state_current)
            push!(forecast_rmse_hybrid_statewise, sqrt(mean((truth_now .- xbar_hyb_f).^2)))
            push!(forecast_mean_hybrid_statewise, xbar_hyb_f)

            # weak coupling at obs times
            ...
            push!(analysis_rmse_hybrid_statewise, sqrt(mean((truth_now .- xbar_hyb_a).^2)))
    ...
    return (
        ...
        λ_state_mat = λ_state_mat,
        A_state_mat = A_state_mat,
        B_state_mat = B_state_mat,
        C_state_mat = C_state_mat
    )
end

results_all_states = run_statewise_hybrid_single_setup();

#for plotting
statewise_single_forcing(results_all_states);
